...
import hiliteBg from '../../assets/img/hilitebg.jpg';
	...
	({ isProtected, title, description, className, children }, ref) => {
		useLayoutEffect(() => {
			...
			document.getElementsByTagName('TITLE')[0].text = `${title ? `${title} | ` : ''}${
				import.meta.env.VITE_API_SITE_NAME
			}`;
			// @ts-ignore
			document
				?.querySelector('meta[name="description"]')
				.setAttribute('content', description || import.meta.env.VITE_API_META_DESC || '');
		});
		...
		return (
			<div ref={ref} className={classNames('page-wrapper', 'container-fluid', className)} 
			 style={className==="hilitebg-banner"?
			{backgroundImage:`url(${hiliteBg})`, 
