...
const useAsideTouch = () => {
	...
	const mobileDesign =
		...
		deviceScreen?.width <= Number(import.meta.env.VITE_API_MOBILE_BREAKPOINT_SIZE);
	const hasTouchButton =
		typeof deviceScreen?.width !== 'undefined' &&
		deviceScreen?.width > Number(import.meta.env.VITE_API_ASIDE_MINIMIZE_BREAKPOINT_SIZE);

	const asideWidthWithSpace =
		(parseInt(String(import.meta.env.VITE_API_ASIDE_WIDTH_PX), 10) +
			parseInt(String(import.meta.env.VITE_API_SPACER_PX), 10)) *
		-1;

	const x = useMotionValue(
		import.meta.env.VITE_API_ASIDE_TOUCH_STATUS === 'true' ? 0 : asideWidthWithSpace,
	);
