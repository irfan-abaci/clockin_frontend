...
export const ThemeContextProvider: FC<IThemeContextProviderProps> = ({ children }) => {
	...
	// @ts-ignore
	const mobileDesign = deviceScreen?.width <= import.meta.env.VITE_API_MOBILE_BREAKPOINT_SIZE;
	...
	const [darkModeStatus, setDarkModeStatus] = useState(
		localStorage.getItem('facit_darkModeStatus')
			...
			: import.meta.env.VITE_API_DARK_MODE === 'true',
	);
	...
	const [asideStatus, setAsideStatus] = useState(
		localStorage.getItem('facit_asideStatus')
			...
			: // @ts-ignore
				deviceScreen?.width >= import.meta.env.VITE_API_ASIDE_MINIMIZE_BREAKPOINT_SIZE,
	);
	useLayoutEffect(() => {
	...
	useLayoutEffect(() => {
		// @ts-ignore
		if (deviceScreen?.width >= import.meta.env.VITE_API_ASIDE_MINIMIZE_BREAKPOINT_SIZE) {
