...
export const getFirstLetters = (text: string, letterCount = 2): string => {
			const words = text.split(/\s+/);
		  
			if (words.length === 1) {
			  // If only one word, take the first two letters
			  return words[0].substring(0, letterCount).toUpperCase();
			} 
			  // If multiple words, take the first letter of each word and concatenate
			  return words
				.map(word => word[0])
				.join('')
				.substring(0, letterCount)
