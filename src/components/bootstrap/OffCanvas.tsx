...
const OffCanvas: FC<IOffCanvasProps> = ({
	...
	/** Clicks on portaled overlays (date range, selects, popovers) must not dismiss the offcanvas. */
	const shouldIgnoreBackdropClose = (target: unknown): boolean => {
		if (!(target instanceof Element)) return false;
		return Boolean(
			target.closest('[data-offcanvas-ignore-backdrop]') ||
	...
	const closeCanvas = (event: Event) => {
		const { target } = event;
		const isReactSelect = target instanceof SVGElement;
		if (shouldIgnoreBackdropClose(target)) return;
		if (
			ref.current &&
