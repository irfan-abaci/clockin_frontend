...
const Modal: FC<IModalProps> = ({
	...
	const isReactSelectClick = (target: EventTarget | null): boolean => {
	...
	const closeModal = (event: { target: any }) => {
		// @ts-ignore
		if (ref.current && !ref.current.contains(event.target) && !isStaticBackdrop&& !isReactSelectClick(event.target)) {
			setIsOpen(false);
		}
	};
